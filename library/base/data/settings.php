<?php
class bv48v_data_settings extends bv48v_base {
	/**
	 * option
	 *
...
*****************************************************************************************/
	...
	public static function config($filename) {
		...
		$data = bv48v_data_array::merge ( $data );
		if(isset($data->settings))
		{
			bv48v_data_array::objects_to_array($data->settings);
		}
		unset ( $data->priority );
	...
	private static function load($file, $legacy = false) {
		...
		if (file_exists ( $file )) {
			switch (pathinfo ( $file, PATHINFO_EXTENSION )) {
				case 'xml' :
					$return = bv48v_data_xml::load ( $file );
					break;
				case 'json' :
					$return = bv48v_data_json::decode ( file_get_contents ( $file ), $legacy );
					break;
			}
		}
		return $return;
	...
	public function data($refresh=false) {
		if (null === $this->_data || $refresh===false) {
			...
			if (null !== $this->_option) {
				$base = new bv48v_data_settings ( $this->application () );
				$settings [] = $base->data ();
			...
			$this->_data = bv48v_data_array::merge ( $settings );
		}
		return $this->_data;
	}
}